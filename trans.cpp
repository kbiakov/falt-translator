	...
	try {
        ...
        for (targetStr = 0; targetStr < linesCount; targetStr++) {
            ...
            if (strstr(fileSource[targetStr], "=") != NULL && isCorrAsns(targetStr)
				&& !strstr(fileSource[targetStr], "цел")
				&& !strstr(fileSource[targetStr], "вещ")
				&& !strstr(fileSource[targetStr], "если")
				&& !strstr(fileSource[targetStr], "пока")) {
					transAssign(targetStr);
            }

			// translate declarations and perhaps definitions of int variables
            if (tmpStr = strstr(fileSource[targetStr], "цел")) {
            ...
            if (tmpStr = strstr(fileSource[targetStr], "вещ")) {
                tmp = fileSource[targetStr] - tmpStr;
                if (tmp == NULL) {
            ...
            if (tmpStr = strstr(fileSource[targetStr], "нач")) {
            ...
            if (tmpStr = strstr(fileSource[targetStr], "кон")) {
                if (tmp == 0 && strlen(fileSource[targetStr]) == 3) {
                    ++endNums;
                    transEnd(targetStr);
            ...
            if (tmpStr = strstr(fileSource[targetStr], "если")) {
                tmp = fileSource[targetStr] - tmpStr;
                tmpStr = strstr(fileSource[targetStr], "то");
                if (tmpStr == NULL) {
            ...
            if (tmpStr = strstr(fileSource[targetStr], "пока")) {
                tmp = fileSource[targetStr] - tmpStr;
                tmpStr = strstr(fileSource[targetStr], "то");
